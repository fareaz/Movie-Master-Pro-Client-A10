...
const TopRate = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const data = Array.isArray(res.data) ? res.data : res.data.result || res.data.movies || res.data.data || res.data;
        const arr = Array.isArray(data) ? data : [];
        setMovies(arr.slice(0, 5));
      })
      .catch((e) => {
        console.error("Top-rate fetch error:", e);
        setError(e.response?.data?.message || e.message || "Failed to fetch top rated movies");
      })
  ...
  if (!movies.length) return <p className="text-center mt-8 text-gray-600">No top rated movies found.</p>;
          ...
          {movies.map((movie, idx) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  backgroundImage: `linear-gradient(180deg, rgba(2,6,23,0.12), rgba(2,6,23,0.35)), url(${movie.posterUrl || ""})`,
                }}
                role="img"
              ...
              <div className="p-4 sm:p-5">
                ...
                <p className="text-sm text-gray-500 mt-1 line-clamp-2">{movie.plotSummary}</p>
